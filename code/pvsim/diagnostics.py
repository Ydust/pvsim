"""Fault detection and diagnostic metrics."""
...
def detect(measured: pd.Series, expected: pd.Series,
    ...
    if onset is None:
        return {"pi_daily": pd.Series(pi, index=m_daily.index, name="PI"),
                ...
                "fault_type": _text_label('no_evident_fault'), "confidence": "—",
...
def _classify(jump, ramp_rate, feat) -> tuple[str, str]:
    ...
    if hc > 0.25:
        return _text_label('partial_shading'), _text_label('diagnostics_text_5') if hc > 0.4 else _text_label('diagnostics_text_4')
    if hl > 0.12:
        return _text_label('diagnostics_text_2'), _text_label('diagnostics_text_5') if hl > 0.2 else _text_label('diagnostics_text_4')
    if jump > 0.06:
        return _text_label('diagnostics_text'), _text_label('diagnostics_text_5') if jump > 0.12 else _text_label('diagnostics_text_4')
    if ramp_rate > 0.002:
        return _text_label('soiling'), _text_label('diagnostics_text_4')
    return _text_label('accelerated_degradation'), _text_label('diagnostics_text_3')
...
_FAULTS = {
    _text_label('soiling'): lambda ac: inject_soiling(ac, 0.18, 120, 60),
    _text_label('partial_shading'): lambda ac: inject_shading(ac, (7, 10), 0.6, 120),
    _text_label('diagnostics_text'): lambda ac: inject_string_failure(ac, 0.25, 120),
    _text_label('diagnostics_text_2'): lambda ac: inject_inverter_clip(ac, 0.7, 120),
    _text_label('accelerated_degradation'): lambda ac: inject_accelerated_degradation(ac, 0.12, 1),
}
...
def evaluate_suite(tech: CellTechnology, weather: pd.DataFrame,
    ...
    for true_type, fn in _FAULTS.items():
        ...
        true_onset = 1 if true_type == _text_label('accelerated_degradation') else 120
        delay = (dg["onset_day"] - true_onset) if dg["onset_day"] is not None else None
        rows.append({
            _text_label('actual_fault'): true_type, _text_label('diagnosis'): dg["fault_type"],
            _text_label('correct'): "✓" if dg["fault_type"] == true_type else "✗",
            _text_label('detection_delay_in_days'): delay, _text_label('diagnostics_text_6'): round(dg["est_loss"] * 100, 1),
            _text_label('confidence'): dg["confidence"],
        })
